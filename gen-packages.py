...
#   unterm, unterm-spike, cosmic-term-static — the terminal project, a product
#                  of its own and not a catalog program.
...
{rows}
        ...
        <p class="pkg-note">
          Not every program ships on every OS: some are Linux-specific (<code>util-linux</code>, <code>shadow</code>, <code>kmod</code>), others rely on platform APIs that aren't available or portable on Windows (<code>htop</code>, <code>tmux</code>). Support is tracked per program in the table above.
          A Linux ✓ covers six architectures — x86_64, aarch64, armv7l, i686, ppc64le, and riscv64; a macOS ✓ covers Intel and Apple Silicon, and Windows builds are x86_64.
